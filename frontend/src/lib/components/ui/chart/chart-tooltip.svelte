<script lang="ts">
	...
	type TooltipLabelValue = string | number | Date;

	function isTooltipLabelValue(value: unknown): value is TooltipLabelValue {
		return typeof value === "string" || typeof value === "number" || value instanceof Date;
	}

	function defaultFormatter(value: TooltipLabelValue, _payload: TooltipPayload[]) {
		return `${value}`;
	}
	...
	let {
		...
		labelFormatter?:
			| ((value: TooltipLabelValue, payload: TooltipPayload[]) => string | number | Snippet)
			| null;
		formatter?: Snippet<
			[
				{
	...
	const formattedLabel = $derived.by(() => {
		...
		if (!isTooltipLabelValue(value)) return String(value);
