<script lang="ts">
	...
	import { Upload, FileText, TriangleAlert, RotateCcw, Maximize, Minimize } from '@lucide/svelte';
...
<svelte:window
	onkeydown={(e) => {
		if (e.key === 'Escape' && !dialogOpen && !uploadOpen) expanded = false;
	}}
/>
			...
			>
				<div class="flex items-center justify-between gap-2 text-sm">
					<div class="flex items-center gap-2">
                        {#if expanded}
                            <h1 class="text-muted-foreground border-b-2">{data.issue.title}</h1> |
                        {/if}
					...
					<div class="flex items-center gap-2">
						...
						>
							{#if expanded}<Minimize />{:else}<Maximize />{/if}
						</Button>
				...
				<div class="flex gap-4 {expanded ? 'min-h-0 flex-1' : 'h-[70vh]'}">
					<div class="w-64 shrink-0 space-y-1 overflow-y-auto border-r pr-2">
						{#each data.documents as d (d.id)}
							<Item.Root
								class="hover:bg-muted cursor-pointer text-left {selected === d.id
									? 'bg-muted'
									: ''}"
							>
								{#snippet child({ props })}
